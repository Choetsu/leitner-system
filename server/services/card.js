const { Card, User, Tag, CardResponse } = require("../db/models");
...
module.exports = function CardService() {
    return {
        ...
        getQuizz: async function (userId, date) {
            ...
            const responses = await CardResponse.findAll({
                where: {
                    userId: userId,
                },
                attributes: [
        ...
        answerQuizz: async function (cardId, userId, isValid) {
            ...
            let response = await CardResponse.findOne({
                where: {
                    userId: userId,
                    cardId: cardId,
                },
            ...
            if (!response) {
                response = await CardResponse.create({
                    isValid: isValid,
                    userId: userId,
                    cardId: cardId,
                    responseDate: new Date(),
